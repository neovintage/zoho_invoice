...
describe ZohoInvoice::Customer do
  describe "retrieving customers" do
    ...
    it "should return an array of active customers" do
      customer_test('/api/v3/customers', 'successful_customers_response', 1, true, :all_active, @client)
    end

    it "should return an array of inactive customers" do
      customer_test('/api/v3/customers/inactive', 'successful_customers_response', 1, false, :all_inactive, @client)
    end

    it "should return an array of all customers" do
      stub_get('/api/v3/customers').
      ...
      stub_get('/api/v3/customers/inactive').
        with(:query => default_credentials).
        to_return(:status => 200, :body => fixture('successful_customers_response'), :headers => {:content_type => 'application/xml'})

      result = ZohoInvoice::Customer.send('all', @client)
      expect(a_get('/api/v3/customers').with(:query => default_credentials)).to have_been_made
      expect(a_get('/api/v3/customers/inactive').with(:query => default_credentials)).to have_been_made
